...
class SearchBar extends Component
{
    ...
    public function render()
    {
        ...
        if (strlen($this->query) >= 2) {
            ...
            $answers = Answer::where('content', 'like', '%' . $this->query . '%')
                ->with('question')
                ->limit(5)
                ->get();
        ...
        return view('livewire.search-bar', compact('questions', 'answers', 'ustaz'));
    }
};
?>
...
<div class="relative" x-data="{ open: false }" @click.outside="open = false">
    <div class="flex items-center gap-2 bg-white border border-slate-200 rounded-xl px-3 py-1.5 shadow-sm">
        ...
        <input
            ...
            @focus="open = true"
            placeholder="Cari pertanyaan, jawaban, ustaz..."
    ...
    @if($query && strlen($query) >= 2)
        ...
        >
            ...
            @if($answers->count())
                <div class="px-3 py-1">
                    <p class="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-2 mt-2">Jawaban</p>
                    @foreach($answers as $a)
                        <a href="{{ route('post.show', $a->question_id) }}" class="block px-2 py-2 rounded-lg hover:bg-slate-50 transition">
                            <p class="text-sm text-slate-700 line-clamp-1">{!! strip_tags($a->content) !!}</p>
                            <p class="text-xs text-slate-400 mt-0.5">{{ $a->created_at->diffForHumans() }}</p>
                        </a>
                    @endforeach
                </div>
                <div class="border-t border-slate-50"></div>
            ...
            {{-- Tidak ada hasil --}}
            @if($questions->isEmpty() && $answers->isEmpty() && $ustaz->isEmpty())
